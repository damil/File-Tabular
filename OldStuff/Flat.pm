...
=begin comment
    ...
    # extraction des mots de la requête (obligatoires, à exclure, autres mots)
    my ($pluswords, $minuswords, $simplewords) = analyse_request($search_string);

    # chaînes à insérer avant et après les mots reconnus 
    my $pre_match = $tdb_param{_pre_match} || "";   # p. ex. "<font color=red>"
    my $post_match = $tdb_param{_post_match} || ""; # p. ex. "</font>"

    if ($search_string =~ /^\+?"?\*"?/) { # si la requête n'est qu'une étoile
	$pre_match = $post_match = "";    # alors pas d'insertion de pré/post
    }

    my $count_lines = 0;	# nombre de lignes retenues

    # fonction qui décide si une ligne est à retenir ou non.
    # si oui, la fonction insère également les chaînes pré/post
    my $matchLine = sub {
	return 0 if $count_lines >= $max_records; 
 	...
 	foreach my $word (@$pluswords) { # pour chaque mot obligatoire
  	...
  	foreach my $word (@$minuswords) { # pour chaque mot à exclure
	    return 0 if /$word/;
	}
	my $r = 0;
    ...
    display_lines(\@records, 
		  ...
		  search_type => "dans les métadonnées", 
...
sub mkRegex {
	...
	else {&error("Requête vide: invalide");}

	$str =~ s/\*/\\w*/g;	# remplacement de l'étoile (regex simplifiée)
	                        #   par la véritable regex
	...
	# remplacement des caractères accentués par des classes
	$str =~ s/ç/[çc]/g;
	$str =~ s/([áàâä])/[a$1]/g;
	$str =~ s/([éèêë])/[e$1]/g;
	$str =~ s/([íìîï])/[i$1]/g;
	...
	if ($field) {	# au cas où le nom de champ est donné
	    my $fn = 0;
	    ...
	    $str = qr/^		# début de ligne
		      (         # capture dans $1
		      (?:[^$FS]*\Q$FS\E){$fn} # saute $fn champs
	              [^$FS]*	# saute début du champ
		      )		# fin $1
		      ($str)  # mot spécifié dans $str, capture dans $2
		     /xi;
	}

	else {	# au cas où l'on cherche sur n'importe quel champ
